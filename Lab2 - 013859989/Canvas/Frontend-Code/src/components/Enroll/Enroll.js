...
var tripDetails = [];
        ...
        SignUpCall2 = (e) => {
         ...
         // var contsa = JSON.parse(localStorage.getItem('sjsuid'));
          //var contsa = localStorage.getItem('sjsuid');
           ...
           axios.post('http://'+rooturl+':3000/Dashboard/enroll/details1',data)//, {
            // headers: {"Authorization" : `Bearer ${token}`}
            // })
               .then(response => {
                   console.log("Status Code : ",response);
        ...
        SignUpCall3 = (e) => {
         ...
         // var contsa = JSON.parse(localStorage.getItem('sjsuid'));
          //var contsa = localStorage.getItem('sjsuid');
           ...
           axios.post('http://'+rooturl+':3000/Dashboard/enroll/details2',data)//, {
            // headers: {"Authorization" : `Bearer ${token}`}
            // })
               .then(response => {
                   console.log("Status Code : ",response);
  ...
  enrollstudent = (param,param1,param2) => (e) => {
    ...
    console.log('check add zalsddsa ka book');
     ...
     //make a post request with the user data
     var token = localStorage.getItem("token");
     axios.post('http://'+rooturl+':3000/Dashboard/enroll/enrollstudent',data, {
      headers: {"Authorization" : `Bearer ${token}`}
      })
         .then(response => {
             console.log("Status Code : ",response);
    ...
    render(){
      ...
      //  let details = [];
        let tripdetails = this.state.books.map(book => {
          return(
              <tr>
                  <td>{book.courseterm}</td>
                  <td>{book.courseid}</td>
                  <td>{book.coursename}</td>
                  <div style={{width: '30%'}}>
                  {this.renderenrollment()}
                  <button 
                    onClick = {this.enrollstudent(book.courseid,book.courseterm,book.coursename)} 
