    ...
    constructor(props){
        ...
        renderRedirect = (e) => {
          ...
          //  console.log(e.target.id);
           ...
           //  e.preventDefault();
             ...
             axios.post('http://'+rooturl+':3000/Dashboard/quizcreation',data, {
              headers: {"Authorization" : `Bearer ${token}`}
                 ...
                 var token = localStorage.getItem("token");
                    //Download image
                    axios.post('http://'+rooturl+':3000/download-file/' + data.ProfileImage, {
                      headers: {"Authorization" : `Bearer ${token}`}
                      })
                        .then(response => {
        ...
        QuizCreationCall = (e) => {
            ...
            console.log('check add zala ka book');
             ...
             //make a post request with the user data
             var token = localStorage.getItem("token");
             axios.post('http://'+rooturl+':3000/Dashboard/quizsubmit',data, {
              headers: {"Authorization" : `Bearer ${token}`}
              })
                 .then(response => {
                     console.log("Status Code : ",response);
